...
void glmGeometryBuilder::load(Json::Value &_jsonRoot, glmTile & _tile){
    ...
    buildLayer(_jsonRoot, "ocean", _tile, .7);
...
//            feature->add(polyline,lineWidth);
            ...
            polygonJson2Mesh(geometryJson["coordinates"], *feature, minHeight, height, _layerName);
            ...
            if (propsJson.isMember("name") && labelManager != NULL) {
                ...
                for (int j = 0; j < geometryJson["coordinates"].size(); j++) {
                    ...
                    polygonJson2Mesh(geometryJson["coordinates"][j], *feature, minHeight, height, _layerName);
                }
...
//---------------------------------------------------------------------------
void glmGeometryBuilder::polygonJson2Mesh(Json::Value &polygonJson, glmMesh &_mesh, float _minHeight, float _height, std::string title) {
    uint16_t indexOffset = (uint16_t)_mesh.getVertices().size();    // track indices
    
    // jrw hack - check for layer name
    if (title == "buildings")
    {
        if (_height > 1)
            h = _height * 1;
    ...
    for (int i = 0; i < polygonJson.size(); i++) {
        ...
        lineJson2Polyline(polygonJson[i],ringCoords,h);
        ...
        if (h != _minHeight){
            ...
            for (int i = 0; i < ringCoords.size() - 1; i++) {
                ...
                _mesh.addVertex(ip0+glm::vec3(0.,0., _minHeight - h - m_geometryOffset.z));
                _mesh.addNormal(nor);
                _mesh.addTexCoord(glm::vec2(0.,1.));
                _mesh.addVertex(ip1+glm::vec3(0.,0., _minHeight - h - m_geometryOffset.z));
                _mesh.addNormal(nor);
