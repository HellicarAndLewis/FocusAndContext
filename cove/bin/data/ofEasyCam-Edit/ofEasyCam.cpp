...
void ofEasyCam::updateRotation(){
	if(bApplyInertia){
		...
		curRot = ofQuaternion(0, getXAxis(), yRot, getYAxis(), 0, getZAxis());
		...
		curRot = ofQuaternion(0, prevAxisX, yRot, prevAxisY, 0, prevAxisZ);
...
#ifdef TOUCH_ENABLED
		yRot = mouseVel.y * sensitivityRotY * 180 / min(viewport.width, viewport.height);
#else
		yRot = -mouseVel.x * sensitivityRotX * 180 / min(viewport.width, viewport.height);
#endif
