...
void main() {
    ...
    vec2 q = glPos.xy / noiseMult;
    vec2 p = -1.0 + 2.0 * q;
    vec2 qq;
    vec2 r;
    float color = pattern2(p,qq,r,time);
    //float color = snoise(vec2(time * q.y, time * q.x));

    float g = 0.8 - (color * 0.3);
    float b = 1.0 - (color * 0.5);
    vec4 c = vec4(0.0, g, b, 1.0);
    //c *= 3.5;
