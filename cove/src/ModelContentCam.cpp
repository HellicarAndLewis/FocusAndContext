...
// when an ModelContentCam is moving due to momentum, this keeps it
...
//----------------------------------------
ModelContentCam::ModelContentCam(){
    lastTap    = 0;
    lastDistance = 0;
    drag = 0.9f;
...
//----------------------------------------
void ModelContentCam::update(ofEventArgs & args){
...
//----------------------------------------
void ModelContentCam::begin(ofRectangle _viewport){
    if(!bEventsSet){
        setEvents(ofEvents());
    }
...
//----------------------------------------
void ModelContentCam::setTarget(const ofVec3f& targetPoint){
    target.setPosition(targetPoint);
    lookAt(target);
}

//----------------------------------------
void ModelContentCam::setTarget(ofNode& targetNode){
    target = targetNode;
    lookAt(target);
}

//----------------------------------------
ofNode& ModelContentCam::getTarget(){
    return target;
}

//----------------------------------------
void ModelContentCam::setDistance(float distance){
    setDistance(distance, true);
}

//----------------------------------------
void ModelContentCam::setDistance(float distance, bool save){//should this be the distance from the camera to the target?
    if(distance > 0.0f){
        if(save){
...
//----------------------------------------
float ModelContentCam::getDistance() const{
    return target.getPosition().distance(getPosition());
}

//----------------------------------------
void ModelContentCam::setAutoDistance(bool bAutoDistance){
...
//----------------------------------------
void ModelContentCam::setDrag(float drag){
    this->drag = drag;
}

//----------------------------------------
float ModelContentCam::getDrag() const{
    return drag;
}

//----------------------------------------
void ModelContentCam::setTranslationKey(char key){
    doTranslationKey = key;
}

//----------------------------------------
char ModelContentCam::getTranslationKey(){
    return doTranslationKey;
}

//----------------------------------------
void ModelContentCam::enableMouseInput(){
    if(!bMouseInputEnabled && events){
        ofAddListener(events->update, this, &ModelContentCam::update);
        ofAddListener(events->mouseDragged , this, &ModelContentCam::mouseDragged);
        ofAddListener(events->mousePressed, this, &ModelContentCam::mousePressed);
        ofAddListener(events->mouseReleased, this, &ModelContentCam::mouseReleased);
        ofAddListener(events->mouseScrolled, this, &ModelContentCam::mouseScrolled);
    }
...
//----------------------------------------
void ModelContentCam::disableMouseInput(){
    if(bMouseInputEnabled && events){
        ofRemoveListener(events->update, this, &ModelContentCam::update);
        ofRemoveListener(events->mouseDragged, this, &ModelContentCam::mouseDragged);
        ofRemoveListener(events->mousePressed, this, &ModelContentCam::mousePressed);
        ofRemoveListener(events->mouseReleased, this, &ModelContentCam::mouseReleased);
        ofRemoveListener(events->mouseScrolled, this, &ModelContentCam::mouseScrolled);
    }
...
//----------------------------------------
void ModelContentCam::setEvents(ofCoreEvents & _events){
...
//----------------------------------------
bool ModelContentCam::getMouseInputEnabled(){
    return bMouseInputEnabled;
}

//----------------------------------------
void ModelContentCam::enableMouseMiddleButton(){
    bEnableMouseMiddleButton = true;
}

//----------------------------------------
void ModelContentCam::disableMouseMiddleButton(){
    bEnableMouseMiddleButton = false;
}

//----------------------------------------
bool ModelContentCam::getMouseMiddleButtonEnabled(){
    return bEnableMouseMiddleButton;
}

//----------------------------------------
void ModelContentCam::updateTranslation(){
    if(bApplyInertia){
        moveX *= drag;
        moveY *= drag;
        moveZ *= drag;
...
//----------------------------------------
void ModelContentCam::updateRotation(){
    if(bApplyInertia){
        xRot *=drag;
        yRot *=drag;
        zRot *=drag;
...
void ModelContentCam::mousePressed(ofMouseEventArgs & mouse){
...
void ModelContentCam::mouseReleased(ofMouseEventArgs & mouse){
...
void ModelContentCam::mouseDragged(ofMouseEventArgs & mouse){
    mouseVel = mouse  - lastMouse;
    
    updateMouse(mouse);
}

void ModelContentCam::mouseScrolled(ofMouseEventArgs & mouse){
    ofRectangle viewport = getViewport(this->viewport);
    prevPosition = ofCamera::getGlobalPosition();
    prevAxisZ = getZAxis();
...
void ModelContentCam::updateMouse(const ofMouseEventArgs & mouse){
    ofRectangle viewport = getViewport(this->viewport);
    int vFlip;
    if(isVFlipped()){
        vFlip = -1;
    ...
    if(bDoTranslate){
        ...
        zRot = 0;
        if(mouseFlipped) {
            yRot = mouseVel.y * rotationFactor;
        } else {
