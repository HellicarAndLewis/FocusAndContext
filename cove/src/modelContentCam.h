...
/*
 ...
 This is a slightly ugly way of modifying ofEasyCam functionality. Unfortunately all the members of ofEasyCam are private and so by extending the class I can't actually do anything. In order so solve this I;ve just completely cloned by own version of the class and given it a new name. This means that I can do whatever I want!
 
 As a result this is an ofEasyCam that is exactly the same as normal ofEasyCams except for 3 key differences:
 1. It can only rotate objects in the Y axis.
 2. It has a variable entitled MouseFlipped which I can set to change the Yrotation orientation if the mouse is flipped like we have in the case of our crappy touchscreen.
 3. All the private members are now protected! Be the change you want to see in the world!
 
 Wish there was a better way to do this and should post on the forum about it!
 
 */


/// \brief A super simple camera for interacting with objects in 3D space.
class ModelContentCam : public ofCamera {
public:
    ...
    bool getMouseFlipped() { return mouseFlipped; };
    
    bool setMouseFlipped(bool _mouseFlipped) { mouseFlipped = _mouseFlipped; };
